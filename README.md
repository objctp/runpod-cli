...
rp endpoint create --name glm-ocr --template <glm-ocr-template-id> \
    --network-volume ocr-models --gpus-from-volume ocr-models \
    --workers-min 0 --workers-max 3 --idle 600
rp endpoint scale <id> --min 0 --max 1
```
...
`template create` and `endpoint create` are **idempotent by name** — a re-run
...
`endpoints` / `pods` (list each), `make destroy` (lists everything you may want
to tear down).

#!/usr/bin/env bash
#
# `rp api` — raw REST / data-plane escape hatch over rp::api_call (the same
# transport every other command uses). Thin facade: it resolves the method,
# path, plane, body, and optional jq filter, then delegates to rp::http /
# rp::http_api — all curl, auth, timeout, and error policy live in
# lib/transport.sh behind that seam.
# Usage: rp api <METHOD> <path> [--body <json>] [--plane rest|api] [--jq <filter>]
#

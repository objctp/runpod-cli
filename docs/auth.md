...
- [`rp auth use`](auth-use.md) — 
- [`rp auth login`](auth-login.md) — Store a RunPod API key as an account (additive — does not replace others) and
- [`rp auth logout`](auth-logout.md) — Remove an account. If it was active and other accounts remain, the active
...
- [`rp auth status`](auth-status.md) — Show the active account and whether an API key is configured and where it is
